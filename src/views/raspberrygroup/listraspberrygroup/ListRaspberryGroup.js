import { cibRaspberryPi, cilFeaturedPlaylist } from '@coreui/icons'
import CIcon from '@coreui/icons-react'
import { CCol, CContainer, CHeaderBrand, CListGroup, CListGroupItem, CRow } from '@coreui/react'
import { AccessAlarms, AddCircle, CloudSync, Edit } from '@mui/icons-material'
...
const ListRaspberryGroup = (props) => {
  ...
  const createNewGroup = () => {
    mainContext.setShowLoader(true)
    axiosInstance
      .post('/saveRaspberryGroup', { groupName: groupName })
      .then((res) => {
        handleResponse(res)
        setShowCreateModal(false)
        loadGroups()
  ...
  const onClickDeleteButton = () => {
    mainContext.showModal(
      'Delete group',
      'Are sure delete raspberry group?',
  ...
  const onClickDeleteButtonYes = () => {
    ...
    const ids = props.grouplist
      ...
      .map((x) => x.id)
    axiosInstance
      .post('/deleteRaspberryGroups', ids)
      .then((res) => {
        handleResponse(res)
        props.setGrouplist(
          props.grouplist
  ...
  const onClickRaspListButton = (playlist) => {
    props.setSelectedGroup([playlist.name])
    props.onClickViewButton()
  }

  const onClickPlaylistButton = (group) => {
    ...
    <>
      ...
      {props.grouplist.length === 0 ? (
        <div style={{ margin: '22px 0px 0px 21px' }}>
          There is no raspberry group. Please create a new group
        </div>
      ) : (
        <div>
            ...
            {props.grouplist.map((group, idx) => (
              ...
              >
                <CRow>
                  ...
                  <CCol xs="7" style={{ lineHeight: '40px' }} onClick={() => onClickGroup(group)}>
                    {group.name}
                  </CCol>
                  <CCol
                    xs="4"
                  ...
                  >
                    <span style={{ color: 'black' }} onClick={() => onClickRaspListButton(group)}>
                      {/* <KeyboardDoubleArrowRightIcon /> */}
                      <CIcon icon={cibRaspberryPi} style={{ color: 'rgb(223 11 11)' }} />
                    </span>
                    <span
                      onClick={() => onClickPlaylistButton(group)}
...
export default ListRaspberryGroup
